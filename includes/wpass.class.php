...
if(!class_exists('CUSTOMIZEADMINMENU')) {
    class CUSTOMIZEADMINMENU extends APPSAZADMIN {
        ...
        {
            add_submenu_page( WPADMINPANEL_MENU_SLUG , __('Manage Admin Menu', 'appsaz-admin'), __('Manage Admin Menu', 'appsaz-admin'), 'manage_options', 'admin_menu_management', array($this, 'wps_admin_menu_management') );
        }

        public function load_menu_assets($nowpage)
        {
          global $wps_pages_slugs;
          if($wps_pages_slugs['wps_manage_menus']=='APPSAZADMIN_page_admin_menu_management') {
            wp_enqueue_script('jquery-ui-sortable');
            wp_enqueue_script( 'wps-sorting', WPADMINPANEL_DIR_URI . 'assets/js/sortjs.js', array( 'jquery' ), '', true );
            wp_enqueue_style('iconPicker-styles', WPADMINPANEL_DIR_URI . 'assets/icon-picker/css/icon-picker.css', '', WPAPPSAZADMIN_VERSION);
            wp_enqueue_style('Appsaz-styles', WPADMINPANEL_DIR_URI . 'assets/css/appsaz-style.css', '', WPAPPSAZADMIN_VERSION);
            wp_enqueue_script( 'iconPicker', WPADMINPANEL_DIR_URI . 'assets/icon-picker/js/icon-picker.js', array( 'jquery' ), '', true );
          }
        }
        ...
        ?>
            <div class="wrap wps-wrap">
              ...
              if( isset( $wps_menu_access ) && $wps_menu_access == 3 && !empty( $wps_privilege_users ) && !in_array( $current_user_id, $wps_privilege_users ) )
                { ?>
                  <div id="message" class="notice notice-error">
                    <h3><?php _e("You don't have enough permission to access this page.", 'appsaz-admin'); ?></h3>
                  </div>
                <?php
                }
              else {
              ...
              <div class="wps-adminmenu-customizer">
                ...
                <?php
                echo __('to customize who can access to all menu items.', 'appsaz-admin');

                echo '<a href="' . admin_url() . 'admin.php?page='. WPADMINPANEL_MENU_SLUG .'#app_options_tab9"><strong>';
                echo __('Click here ', 'appsaz-admin');
                echo '</strong></a>';
                ?>
                </p></div>
                ...
                <div class="manage_admin_menu_sorter">
                   ...
                   <!--   <div class="wps-manage-menu-slug-mthd">
                        <h2><?php /*_e('Choose method', 'appsaz-admin'); */?></h2>
                        <input type="radio" name="wps_manage_menu_slug_mthd" value="1" <?php /*if( $wps_manage_menu_mthd == 1 ) echo 'checked=checked'; */?> /><label for="default"><?php /*_e('Default', 'appsaz-admin'); */?></label>
                        <p><b><?php /*_e('Note: By switching methods, you will loose your previous admin menu settings.', 'appsaz-admin'); */?></b></p>
                      </div>-->
                                ...
                                <div class="alter-menu-contents" id="s">
                                  ...
                                  <?php if(!empty($value[0])) : ?>
                                    <div class="menu_title">
                                        <label for="menu_title"><em><?php _e('Rename Title', 'appsaz-admin'); ?></em></label>
                                    ...
                                    <div class="menu_icon">
                                        <label for="icon_picker"><em><?php _e('Choose Icon', 'appsaz-admin'); ?></em></label>
                    ...
                    <input type="submit"  class="button button-primary button-hero" value="<?php esc_html_e('Save Changes', 'appsaz-admin'); ?>" />
                    </form>
                </div>
            </div>
          </div>
        ...
        public function Menu_title($title)
        {
            ...
            if (__('Separator', 'appsaz-admin') == $title) {
          ...
          */
          ...
          if( $clean_url === true ) {
            ...
            $output = '<div class="hide-for-roles">' .
                '<label class="hide-for-roles" for="hide-for-roles"><em>' . __('Hide menu for', 'appsaz-admin') . '</em></label>';
