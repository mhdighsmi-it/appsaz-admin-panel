jQuery(function() {
   ...
   jQuery("a.admin_menu_edit").on('click', function(e) {
    ...
    jQuery("#btn-add-menu").click(function(){
        //alert('t');
        jQuery('.content-loader').toggle('slow');
    });

    jQuery("#add_menu").click(function() {
       var no = jQuery('#top_menu').children().length;
       var menu_title = jQuery('#new_menu_title').val();
       var menu_slug = jQuery('#new_menu_slug').val();
       var menu_icon = jQuery('#menu-icon-for-99').val();
       if(typeof menu_icon == "undefined"){
           menu_icon = "";
       }

       var row = jQuery('#template').val();
       row = row.replace(/{sl}/g, no);
       row = row.replace(/{new_menu_title}/g, menu_title);
       row = row.replace(/{menu_icon_class}/g, menu_icon);
       row = row.replace(/{new_menu_slug}/g, menu_slug);
       jQuery('#top_menu').append(row);
       admin_menu_reorder();
